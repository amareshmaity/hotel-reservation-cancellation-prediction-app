...
from src.utils.common import read_yaml, load_data
...
class DataProcessor:
    ...
    def preprocess_data(self, df):
        try:
            ...
            raise CustomException("Error while proprocess data", e)
    ...
    def balance_data(self, df):
        try:
            ...
            balanced_df = pd.DataFrame(X_resampled, columns=X_resampled.columns)
    ...
    def save_data(self, df, file_path):
        try:
            ...
            logger.error(f"Error during data saving step after proprocessing {e}")
            raise CustomException("Error while saving preprocessed data", e)      


    def process(self):
        try:
            logger.info("Loading dta from RAW directory")
...
if __name__ == "__main__":
    processor = DataProcessor(TRAIN_DATA_FILE_PATH, TEST_DATA_FILE_PATH, PROCESSED_DIR, CONFIG_PATH)
    processor.process()